...
- **Auto-retaliate**: When an enemy lands a hit on the player and `player.current_action is None`, the player automatically sets `current_action = "attacking"` targeting that enemy. Since the enemy is adjacent, the next 600ms tick fires melee immediately (classic RS default).
- **Enemy AI**: Enemies chase player within their `aggro_range` but stop when within `TILE_SIZE + 4` (36px) of the player center — they stand adjacent, not on top of the player. Collision damage = `random.randint(1, enemy.max_hit)` reduced by player defense (1s cooldown). Each enemy type has its own aggro range and speed.
